                        ...
                        for(var method in object){ //console.log('loop:: ','method ',method,' :: Element ',(name) ? name : key,'>> type', typeof object[method]);
                            ...
                            if(object.hasOwnProperty(method)){
                                if(typeof object[method] === 'function'){
                                    ...
                                    arrFunc = arrObj;
                                    arrFunc.push(method+'_f');

                                    var arr = arrFunc.join('.');
                                    obj.methods[arr] = object[method].toString();

                                    console.log('Final >> ArrObj',arrObj);
                                    if(arrObj.length > 0){arrObj.pop();arrObj.pop();}
                                    console.log('TRUE Final >> ArrObj',arrObj);
                                    //arrFunc=[];
                                }else if(typeof object[method] === 'object'){
                                    if(arrObj[arrObj.length] !== method){
                                        arrObj.push(method+'_o');
                                    }
                                    console.log('ArrObj',arrObj);
                                    console.log('|||||||||--Object Check,',object[method],((name)? name: key),method);
                                    objectCheck(object[method],((name)? name: key),method);
