var CodeFlower = function(selector, w, h) {
  ...
  this.force = d3.layout.force()
    ...
    .charge(function(d) {return d._children ? -d.size / 100 : -((w+h)/20); })
    .linkDistance(function(d) { return d.target._children ? 80 : 10; })
    .size([h, w]);
...
CodeFlower.prototype.update = function(json) {
  ...
  this.force
    .gravity(Math.atan(total / 100) / Math.PI * 0.4)
    .nodes(nodes)
    .links(links)
    .start();
  ...
  // Update the links 2
  this.linky = this.svg.selectAll("line.link")
    .data(links, function(d) { return d.target.name; });

  // Enter any new links
  this.link.enter().insert("svg:line", ".node")
    .attr("class", "link")
    .attr("x1", function(d) { _t.nodeParse(d); return d.source.x; })
  ...
  console.log('LINK',this.link.enter());

  // Exit any old links.
  this.link.exit().remove();
  this.linky.exit().remove();
...
CodeFlower.prototype.nodeParse = function(data) {

    this.coordObject.push(data);

    //return object;
};
CodeFlower.prototype.coordObject = [];
